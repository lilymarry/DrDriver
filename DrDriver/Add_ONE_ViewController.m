...
@interface Add_ONE_ViewController ()<UIActionSheetDelegate,UIImagePickerControllerDelegate,UINavigationControllerDelegate,UITextFieldDelegate,GYZChooseCityDelegate,CityListViewDelegate>
{
    NSInteger alertType;
    ...
    NSInteger photoType;//上传类型 1:头像   2:行驶证 3:从业资格证 4：身份证
    BOOL isSex;//是否是选择性别
...
@implementation Add_ONE_ViewController
...
//        self.Address_Label.text = @"获取位置失败，请选择手动获取";
//    }else{
//
//        self.Address_Label.text = [NSString stringWithFormat:@"%@(所在城市)",_choosedCity];
//    }
    
    if ([self.faceState isEqualToString:@"photo"]) {
        
    }else{
        if ([self.faceState isEqualToString:@"fail"]) {
            [AFRequestManager postRequestWithUrl:DRIVER_REGISTER_INFO params:@{@"driver_id":_driver.driver_id,
                                                                               @"token":_driver.token} tost:YES special:0 success:^(id responseObject) {
                NSLog(@"%@",responseObject);
                self.ModelDic = responseObject[@"data"];
                self.driver_class = responseObject[@"data"][@"driver_class"];
                self.choosedCity = responseObject[@"data"][@"city_name"];
                [self setDriverDate];
...
-(void)setDriverDate{
  //    _choosedCity = _ModelDic[@"city_name"];
        NSLog(@"%@",_ModelDic);
    self.Alert_Label.text = _ModelDic[@"audit_fail_reason"];
    NSString *headUrl = _ModelDic[@"driver_head"];
    NSString *DriveCardUrl = _ModelDic[@"license_pic"];
    NSString *PermitUrl = _ModelDic[@"employ_pic"];
    NSString *IdcardURL = _ModelDic[@"idcards_person_pic"];

    if (headUrl.length>0) {
        [_headImage sd_setImageWithURL:[NSURL URLWithString:headUrl] completed:^(UIImage * _Nullable image, NSError * _Nullable error, SDImageCacheType cacheType, NSURL * _Nullable imageURL) {
            if (error.code == -1009) {
                [CYTSI otherShowTostWithString:@"网络错误，请检查网络"];
                [self.navigationController popViewControllerAnimated:YES];
            }else{
                [self.DateDic setObject:image forKey:@"Head_PIC"];
            }
        }];
    }
    if (DriveCardUrl.length>0) {
        [_DriveCard_BTN sd_setImageWithURL:[NSURL URLWithString:DriveCardUrl] forState:UIControlStateNormal completed:^(UIImage * _Nullable image, NSError * _Nullable error, SDImageCacheType cacheType, NSURL * _Nullable imageURL) {
            if (error.code == -1009) {
                ...
                [self.DateDic setObject:image forKey:@"DriveCard_PIC"];
            }

        }];
    }
    if (PermitUrl.length>0) {
        [_Permit_BTN sd_setImageWithURL:[NSURL URLWithString:PermitUrl] forState:UIControlStateNormal completed:^(UIImage * _Nullable image, NSError * _Nullable error, SDImageCacheType cacheType, NSURL * _Nullable imageURL) {
            if (error.code == -1009) {
                
            }else{
                [self.DateDic setObject:image forKey:@"Permit_PIC"];
            }
            
        }];
    }
    NSLog(@"IdcardURL ======= %@",IdcardURL);
    if (IdcardURL.length>0) {
        [_IdCar_BTN sd_setImageWithURL:[NSURL URLWithString:IdcardURL] forState:UIControlStateNormal completed:^(UIImage * _Nullable image, NSError * _Nullable error, SDImageCacheType cacheType, NSURL * _Nullable imageURL) {
            if (error.code == -1009) {
                
            }else{
                [self.DateDic setObject:image forKey:@"IdCar_PIC"];
            }
            
        }];
    }
    self.Name_Field.text = _ModelDic[@"driver_name"];
    _IDCard_Field.text = _ModelDic[@"driver_number"];
    _DriveNumber_Field.text = _ModelDic[@"driver_license"];
    if([_ModelDic[@"driver_sex"] isEqualToString:@""]){
       _Sex_Label.text = @"男";
    }else if ([_ModelDic[@"driver_sex"] isEqualToString:@"0"]) {
        _Sex_Label.text = @"女";
    }else if([_ModelDic[@"driver_sex"] isEqualToString:@"1"]){
        _Sex_Label.text = @"男";
    }else{
        _Sex_Label.text = @"保密";
        
    }
    _CompanyName_Field.text = _ModelDic[@"company_name"];
}
//创建导航栏
-(void)setUpNav
{
...
-(void)imagePickerController:(UIImagePickerController *)picker didFinishPickingMediaWithInfo:(NSDictionary *)info
{
    ...
    switch (photoType) {
        ...
        case 103:
        {
            ...
            [_IdCar_BTN setImage:image forState:UIControlStateNormal];
            [self.DateDic setObject:image forKey:photoTypeStr];
        }
...
//    }
    ...
    Add_TWO_ViewController * RVC = [board instantiateViewControllerWithIdentifier:@"Add_TWO_id"];
    RVC.DrivedateDic = _DateDic;
    RVC.driver=_driver;
    RVC.ModelDic = _ModelDic;
...
//    self.View_3.layer.borderWidth=0.5;
    ...
    self.View_12.layer.masksToBounds = YES;
    self.View_12.layer.cornerRadius=5;
    self.View_12.layer.borderColor=[CYTSI colorWithHexString:@"#cccccc"].CGColor;;
    self.View_12.layer.borderWidth=0.5;
