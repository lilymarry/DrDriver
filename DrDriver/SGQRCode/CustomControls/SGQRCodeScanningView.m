...
- (void)setupSubviews {
    ...
    CGFloat scanContent_layerW = DeviceWidth - 2 * scanContent_X;
    CGFloat scanContent_layerH = scanContent_layerW;
    
//    NSLog(@"屏幕宽：%f",[UIScreen mainScreen].bounds.size.width);
//    NSLog(@"左：%f,上：%f,宽：%f,高：%f",scanContent_layerX,scanContent_layerY,scanContent_layerW,scanContent_layerH);
...
#pragma mark - - - 扫描外部View的创建
    ...
    CGFloat top_layerW = DeviceWidth;
    ...
    CGFloat bottom_layerW = DeviceWidth;
    CGFloat bottom_layerH = DeviceHeight - bottom_layerY;
    ...
    CGFloat promptLabelW = DeviceWidth;
    ...
    CGFloat light_buttonW = DeviceWidth;
...
- (UIImageView *)scanningline {
    if (!_scanningline) {
        ...
        _scanningline.frame = CGRectMake(scanContent_X * 0.5, scanContent_Y, DeviceWidth - scanContent_X , scanninglineHeight);
    }
...
- (void)timeAction {
    ...
    if (flag) {
        ...
        if (_scanningline.frame.origin.y >= scanContent_Y) {
            CGFloat scanContent_MaxY = scanContent_Y + DeviceWidth - 2 * scanContent_X;
