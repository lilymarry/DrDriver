...
-(void)creatHttp
{
    ...
    [AFRequestManager postRequestWithUrl:DRIVER_BASE_DATA params:@{@"driver_id":[[NSUserDefaults standardUserDefaults] objectForKey:@"userid"],@"token":[[NSUserDefaults standardUserDefaults] objectForKey:@"token"],@"p":currentPage} tost:YES special:0 success:^(id responseObject) {
        ...
        self.headerView.frame = CGRectMake(0, 0, DeviceWidth, topHeight + 113);
        self.headerView.noticeLB.text = responseObject[@"data"][@"notice"];
...
{
    NSLog(@"indexrow%ld   row: %ld",(long)indexPath.section,(long)indexPath.row);
    ...
    if (indexPath.section==0 && nowOrderArray.count!=0) {
        ...
        NSLog(@"indexPath.rowindexPath.rowindexPath.rowindexPath.rowindexPath.row%ld   row: %ld",(long)indexPath.section,(long)indexPath.row);
            if (completeOrderArray.count == 0 || indexPath.row == completeOrderArray.count) {
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (indexPath.section==0 && nowOrderArray.count!=0) {
        ...
        if (completeOrderArray.count == 0 || indexPath.row == completeOrderArray.count) {
            ...
            vc.start_province = self.start_province;
            vc.start_city = self.start_city;
            vc.start_county = self.start_county;
            vc.getMainData = ^{
                [weakSelf refreshUp];
            };
            [self.navigationController pushViewController:vc animated:YES];
