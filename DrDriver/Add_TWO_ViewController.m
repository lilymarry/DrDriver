...
@interface Add_TWO_ViewController ()<UIActionSheetDelegate,UIImagePickerControllerDelegate,UINavigationControllerDelegate,UITextFieldDelegate,TimePickerViewDelegate>
{
    NSInteger alertType;
...
@implementation Add_TWO_ViewController
...
- (void)viewDidLoad {
    ...
    self.Alert_Label.text = _ModelDic[@"audit_fail_reason"];
    NSString *ManAndCarUrl = _ModelDic[@"vehicle_pic"];
    NSString *DriveCardUrl = _ModelDic[@"drive_pic"];
    NSString *yunshuzhengUrl = _ModelDic[@"transport_pic"];
    NSString *compulsory_insuranceUrl = _ModelDic[@"compulsory_insurance"];
    NSString *commercial_insuranceUrl = _ModelDic[@"commercial_insurance"];
    NSString *driver_contract1Url = _ModelDic[@"driver_contract1"];
    NSString *driver_contract2Url = _ModelDic[@"driver_contract2"];
    NSString *driver_contract3Url = _ModelDic[@"driver_contract3"];
    
    if (ManAndCarUrl.length>0) {
        [_PIC_BTN sd_setImageWithURL:[NSURL URLWithString:ManAndCarUrl]  forState:UIControlStateNormal completed:^(UIImage * _Nullable image, NSError * _Nullable error, SDImageCacheType cacheType, NSURL * _Nullable imageURL) {
            if (error.code == -1009) {
                [CYTSI otherShowTostWithString:@"网络错误，请检查网络"];
                [self.navigationController popViewControllerAnimated:YES];
            }else{
                [self.DateDic setObject:image forKey:@"manAndCar_PiC"];
            }
        }];
    }
    if (DriveCardUrl.length>0) {
        [_DriveCard_BTN sd_setImageWithURL:[NSURL URLWithString:DriveCardUrl] forState:UIControlStateNormal completed:^(UIImage * _Nullable image, NSError * _Nullable error, SDImageCacheType cacheType, NSURL * _Nullable imageURL) {
            if (error.code == -1009) {
                [CYTSI otherShowTostWithString:@"网络错误，请检查网络"];
                [self.navigationController popViewControllerAnimated:YES];
            }else{
                [self.DateDic setObject:image forKey:@"Drive_PIC"];
            }
        }];
    }
    if (yunshuzhengUrl.length>0) {
        [_yunshuzhengBtn sd_setImageWithURL:[NSURL URLWithString:yunshuzhengUrl] forState:UIControlStateNormal completed:^(UIImage * _Nullable image, NSError * _Nullable error, SDImageCacheType cacheType, NSURL * _Nullable imageURL) {
            if (error.code == -1009) {
                [CYTSI otherShowTostWithString:@"网络错误，请检查网络"];
                [self.navigationController popViewControllerAnimated:YES];
            }else{
                [self.DateDic setObject:image forKey:@"transport_pic"];
            }
            
        }];
    }
    if (compulsory_insuranceUrl.length>0) {
        [_compulsory sd_setImageWithURL:[NSURL URLWithString:compulsory_insuranceUrl] forState:UIControlStateNormal completed:^(UIImage * _Nullable image, NSError * _Nullable error, SDImageCacheType cacheType, NSURL * _Nullable imageURL) {
            if (error.code == -1009) {
                [CYTSI otherShowTostWithString:@"网络错误，请检查网络"];
                [self.navigationController popViewControllerAnimated:YES];
            }else{
                [self.DateDic setObject:image forKey:@"compulsory_pic"];
            }
            
        }];
    }
    if (commercial_insuranceUrl.length>0) {
        [_commercial sd_setImageWithURL:[NSURL URLWithString:commercial_insuranceUrl] forState:UIControlStateNormal completed:^(UIImage * _Nullable image, NSError * _Nullable error, SDImageCacheType cacheType, NSURL * _Nullable imageURL) {
            if (error.code == -1009) {
                [CYTSI otherShowTostWithString:@"网络错误，请检查网络"];
                [self.navigationController popViewControllerAnimated:YES];
            }else{
                [self.DateDic setObject:image forKey:@"commercial_pic"];
            }
            
        }];
    }
    if (driver_contract1Url.length>0) {
        [_contarct1 sd_setImageWithURL:[NSURL URLWithString:driver_contract1Url] forState:UIControlStateNormal completed:^(UIImage * _Nullable image, NSError * _Nullable error, SDImageCacheType cacheType, NSURL * _Nullable imageURL) {
            if (error.code == -1009) {
                [CYTSI otherShowTostWithString:@"网络错误，请检查网络"];
                [self.navigationController popViewControllerAnimated:YES];
            }else{
                [self.DateDic setObject:image forKey:@"contract1_pic"];
            }
            
        }];
    }
    if (driver_contract2Url.length>0) {
        [_contract2 sd_setImageWithURL:[NSURL URLWithString:driver_contract2Url] forState:UIControlStateNormal completed:^(UIImage * _Nullable image, NSError * _Nullable error, SDImageCacheType cacheType, NSURL * _Nullable imageURL) {
            if (error.code == -1009) {
                [CYTSI otherShowTostWithString:@"网络错误，请检查网络"];
                [self.navigationController popViewControllerAnimated:YES];
            }else{
                [self.DateDic setObject:image forKey:@"contract2_pic"];
            }
            
        }];
    }
    if (driver_contract3Url.length>0) {
        [_contract3 sd_setImageWithURL:[NSURL URLWithString:driver_contract3Url] forState:UIControlStateNormal completed:^(UIImage * _Nullable image, NSError * _Nullable error, SDImageCacheType cacheType, NSURL * _Nullable imageURL) {
            if (error.code == -1009) {
                [CYTSI otherShowTostWithString:@"网络错误，请检查网络"];
                [self.navigationController popViewControllerAnimated:YES];
            }else{
                [self.DateDic setObject:image forKey:@"contract3_pic"];
            }
            
        }];
    }
    if (![_ModelDic[@"vehicle_person"] isKindOfClass:[NSNull class]]) {
        NSLog(@"%@",_ModelDic[@"vehicle_person"]);
        self.Name_Field.text = _ModelDic[@"vehicle_person"];
    }
    if (![_ModelDic[@"register_time"] isKindOfClass:[NSNull class]]) {
        _Time_Label.text = _ModelDic[@"register_time"];
        
    }
    if (_Time_Label.text.length<1) {
        _Time_Label.text = @"请选择注册时间";
    }
    // _Number_Field.text = _ModelDic[@"vehicle_number"];
    if (![_ModelDic[@"vehicle_number"] isKindOfClass:[NSNull class]]) {
        _Number_Field.text = _ModelDic[@"vehicle_number"];
    }
    
    //_Brand_Label.text = _ModelDic[@"vehicle_style"];
    if (![_ModelDic[@"vehicle_style"] isKindOfClass:[NSNull class]]) {
        _Brand_Label.text = [NSString stringWithFormat:@"%@ %@",_ModelDic[@"vehicle_brand"],
                             _ModelDic[@"vehicle_style"]];
    }
    
    if (_Brand_Label.text.length<1) {
        _Brand_Label.text = @"请选择车辆品牌";
    }
    if (![_ModelDic[@"vehicle_style"] isKindOfClass:[NSNull class]]) {
        _vehicleStyle =_ModelDic[@"vehicle_style"];
        
    }
    if (![_ModelDic[@"vehicle_brand"] isKindOfClass:[NSNull class]]) {
        _carBrandName = _ModelDic[@"vehicle_brand"];
    }
    if (![_ModelDic[@"vehicle_color"] isKindOfClass:[NSNull class]]) {
        _Color_Label.text = _ModelDic[@"vehicle_color"];
        
    }
    if (_Color_Label.text.length<1) {
        _Color_Label.text = @"请选择车辆颜色";
    }
    
    if (![_ModelDic[@"seat_num"] isKindOfClass:[NSNull class]]) {
        self.hezaiNumber.text = _ModelDic[@"seat_num"];
    }
    if (![_ModelDic[@"certificate"] isKindOfClass:[NSNull class]]) {
        NSLog(@"%@",_ModelDic[@"certificate"]);
        self.yunshuzhengNumber.text = _ModelDic[@"certificate"];
    }
    if (![_ModelDic[@"vin"] isKindOfClass:[NSNull class]]) {
        self.vinTF.text = _ModelDic[@"vin"];
    }
    if (![_ModelDic[@"engineid"] isKindOfClass:[NSNull class]]) {
        self.engineTF.text = _ModelDic[@"engineid"];
    }
...
-(void)imagePickerController:(UIImagePickerController *)picker didFinishPickingMediaWithInfo:(NSDictionary *)info
{
    ...
    switch (photoType) {
        ...
        case 106:
        {
            ...
            [_contarct1 setImage:image forState:UIControlStateNormal];
            [_DateDic setObject:image forKey:photoTypeStr];
...
- (IBAction)didClickToSend:(UIButton *)sender {
    ...
    if ([_Time_Label.text isEqualToString:@"请选择注册时间"] || _Time_Label.text.length<1) {
    ...
    if ([_Brand_Label.text isEqualToString:@"请选择车辆品牌"] || _Brand_Label.text.length<1) {
        [CYTSI otherShowTostWithString:@"请输入车辆品牌"];
        return;
    }
    if ([_Color_Label.text isEqualToString:@"请选择车辆颜色"]|| _Color_Label.text.length<1) {
    ...
    if (self.hezaiNumber.text.length<1 || ![CYTSI isNumber:self.hezaiNumber.text]) {
        [CYTSI otherShowTostWithString:@"请填写核定载客人数"];
        return;
...
-(void)uploadMessage
{
    ...
    NSDictionary *dict = @{@"driver_id":_driver.driver_id,
                           ...
                           @"vehicle_color":_Color_Label.text,
        ...
        for (int i=0; i<12; i++) {
            ...
            if (i<4) {
                ...
                if (_DateDic[imageStr] !=nil) {
                    ...
                    //                    [self saveImage:_DateDic[imageStr] withName:dateStr];
                    NSString *fullPath = [[NSHomeDirectory() stringByAppendingPathComponent:@"Documents"] stringByAppendingPathComponent:dateStr];
                    [formData appendPartWithFileURL:[NSURL fileURLWithPath:fullPath] name:keyStr fileName:dateStr mimeType:@"image/png" error:nil];
