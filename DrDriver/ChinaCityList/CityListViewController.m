...
- (void)viewDidLoad
{
    ...
    if (![CLLocationManager locationServicesEnabled] || [CLLocationManager authorizationStatus]==kCLAuthorizationStatusDenied)
        //位置服务是在设置中禁用
    {
        self.arrayLocatingCity   = [NSMutableArray arrayWithObjects:@"定位失败", nil];
    }else{
        [self locationStart];
    }
...
- (void)setNavigationWithTitle:(NSString *)title
{
    ...
    customNavView.backgroundColor = [UIColor blackColor];
    ...
    [geocoder reverseGeocodeLocation:currentLocation completionHandler:^(NSArray *array, NSError *error)
     {
         if (array.count >0)
         {
             ...
             if (!currCity) {
                ...
                _arrayLocatingCity   = [NSMutableArray arrayWithObjects:currCity, nil];
                 _locatingCityGroupView.items = [self GetCityDataSoucre:_arrayLocatingCity];
         ...
         } else if (error ==nil && [array count] == 0)
         {
             ...
             [CYTSI otherShowTostWithString:@"定位失败，请手动选择"];
         }else if (error !=nil)
         {
