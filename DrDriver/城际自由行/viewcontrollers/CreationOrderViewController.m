...
@interface CreationOrderViewController ()<UIScrollViewDelegate,UITextViewDelegate,UIPickerViewDelegate, UIPickerViewDataSource,AMapSearchDelegate,LeeTagViewDelegate>
{
...
@implementation CreationOrderViewController
...
-(void)creatMainView:(NSDictionary *)dic{
    ...
    //    self.scrollView.contentSize = CGSizeMake(DeviceWidth, 360 + 254 + Bottom_Height);
    ...
    //    self.sumPriceLB.font = [UIFont systemFontOfSize:11];
    ...
    [self.sumPriceLB mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        //        make.right.equalTo(self.bgView.mas_right).offset(-2);
    ...
    self.distanceLB.text = [NSString stringWithFormat:@"本次行程预计%@km,大约行驶%@小时",dic[@"estimate_mile"],dic[@"estimate_time"]];
...
//                self.sumPriceLB.text = [NSString stringWithFormat:@"一口价包车%.2f元",self.driver_fixed_price * [self.priceTF.text floatValue] / self.driver_price];
                self.changeDriver_price = self.priceTF.text;
//                self.changeDriver_fixed_price = [NSString stringWithFormat:@"%.2f",self.driver_fixed_price * [self.priceTF.text floatValue] / self.driver_price];
            }
        }else{
//            self.sumPriceLB.text = [NSString stringWithFormat:@"一口价包车%.2f元",self.driver_fixed_price];
            self.changeDriver_price = @"-";
//            self.changeDriver_fixed_price = [NSString stringWithFormat:@"%.2f",self.driver_fixed_price];
        }
    }
}
...
-(void)OrderButtonClicked{
    ...
    if ([self.priceTF.text floatValue] < self.driver_price * 0.5) {
        [CYTSI otherShowTostWithString:@"价格不能低于推荐价格的50%"];
        return;
    }else if ([self.priceTF.text floatValue] > self.driver_price * 1.5){
        [CYTSI otherShowTostWithString:@"价格不能高于推荐价格的50%"];
        return;
    }
    if ([self.changeDriver_price isEqualToString:@"-"]) {
    ...
    if ([self.sumPriceTF.text floatValue] < self.driver_fixed_price * 0.5) {
        [CYTSI otherShowTostWithString:@"价格不能低于推荐价格的50%"];
        return;
    }else if ([self.sumPriceTF.text floatValue] > self.driver_fixed_price * 1.5){
        [CYTSI otherShowTostWithString:@"价格不能高于推荐价格的50%"];
        return;
    }
...
//-(void)getDistance{
//    if (![self.startDic[@"lat"] isEqualToString:@""] && ![self.startDic[@"lng"] isEqualToString:@""] && ![self.endDic[@"lat"] isEqualToString:@""] && ![self.endDic[@"lng"] isEqualToString:@""]) {
...
//
//    /* 出发点. */
//    navi.origin = [AMapGeoPoint locationWithLatitude:[self.startDic[@"lat"] floatValue]
//                                           longitude:[self.startDic[@"lng"] floatValue]];
//    /* 目的地. */
//    navi.destination = [AMapGeoPoint locationWithLatitude:[self.endDic[@"lat"] floatValue]
//                                                longitude:[self.endDic[@"lng"] floatValue]];
//
//
//
...
//
//    self.distanceLB.text= [NSString stringWithFormat:@"本行程公里数 %.2f km",path.distance / 1000.f];
//}
