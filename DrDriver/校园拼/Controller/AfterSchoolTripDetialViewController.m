...
@interface AfterSchoolTripDetialViewController ()<UITableViewDelegate, UITableViewDataSource, AfterSchoolTableViewCellDelegate, UIImagePickerControllerDelegate, UINavigationControllerDelegate, MAMapViewDelegate, AMapNaviDriveManagerDelegate, DriveNaviViewControllerDelegate, AMapLocationManagerDelegate>
{
...
@implementation AfterSchoolTripDetialViewController
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    AfterSchoolTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:@"AfterSchoolTableViewCell"];
    if (!cell) {
        cell = [[AfterSchoolTableViewCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"AfterSchoolTableViewCell"];
...
- (void)clickPhotoButton:(NSString *)order_id {
...
- (void)skipBtnClick:(NSString *)orderStr withType:(NSString *)type {
    ...
    UIAlertAction *action1 = [UIAlertAction actionWithTitle:@"原地等待" style:UIAlertActionStyleDefault handler:^(UIAlertAction *_Nonnull action) {
        [AFRequestManager postRequestWithUrl:DRIVER_SCHOOLBUS_ACCONPANY params:@{ @"order_id": orderStr, @"driver_id": [[NSUserDefaults standardUserDefaults] objectForKey:@"userid"], @"token": [[NSUserDefaults standardUserDefaults] objectForKey:@"token"], @"type": @"2" } tost:NO special:0 success:^(id responseObject) {
            [weakSelf creatHttp];
        } failure:^(NSError *error) {
        }];
    }];
    UIAlertAction *action2 = [UIAlertAction actionWithTitle:@"送其他乘客" style:UIAlertActionStyleDefault handler:^(UIAlertAction *_Nonnull action) {     //给出弹框
        [AFRequestManager postRequestWithUrl:DRIVER_SCHOOLBUS_ACCONPANY params:@{ @"order_id": orderStr, @"driver_id": [[NSUserDefaults standardUserDefaults] objectForKey:@"userid"], @"token": [[NSUserDefaults standardUserDefaults] objectForKey:@"token"], @"type": @"1" } tost:NO special:0 success:^(id responseObject) {
            [weakSelf creatHttp];
        } failure:^(NSError *error) {
        }];
    }];
...
//            NSLog(@"点击确认");
        ...
        lvc.liveBlock = ^(UIImage *image) {
            dispatch_async(dispatch_get_main_queue(), ^{
                ...
                //1.创建管理者对象
//                NSLog(@"=======%f",_userLocation.coordinate.longitude);
                AFHTTPSessionManager *manager = [AFHTTPSessionManager manager];
