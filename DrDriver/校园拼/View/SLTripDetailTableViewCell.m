...
@interface SLTripDetailTableViewCell()
...
@implementation SLTripDetailTableViewCell
...
//        btn.frame = CGRectMake(DeviceWidth - 62, 8, 50, 24);
      ...
      [bgView addSubview:statuButton];
       ...
       self.passLabelTap  = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(passLabelTapDidClick)];
        self.passLate_label.userInteractionEnabled = YES; //必须开启这一句手势才有效果
        [self.passLate_label addGestureRecognizer:self.passLabelTap];
    }
...
//    NSLog(@"student ===== %@",_student);
    ...
    self.name_Label.text = _student.child_name;
...
//order_state订单状态，0：司机未发车，1司机未到达，2未上车，3已上车，4行程中，5司机已送达，6家长确认送达，7司机取消，8乘客取消 9陪伴中
    if ([_student.order_state isEqualToString:@"0"]) {
        ...
        //一点击到达上车地点，首先出现倒计时五分钟按钮，倒计时完成，乘客迟到
        [self addSubview:self.passLate_label];
        [self.passLate_label mas_makeConstraints:^(MASConstraintMaker *make) {
             make.top.mas_equalTo(self.statu_Label.mas_bottom).mas_offset(10);
             make.left.mas_equalTo(self.mas_left).mas_offset(15);
             make.width.mas_equalTo((DeviceWidth-15*4)/2);
             make.height.mas_equalTo(44);
         }];
       [self.statu_button mas_updateConstraints:^(MASConstraintMaker *make) {
          make.left.mas_equalTo(self.passLate_label.mas_right).mas_offset(15);
      }];
        [self.statu_button updateConstraintsIfNeeded];
     
        //服务器传入时间跟现在时间对比
       self.timeInt = [CYTSI secondWithTimeIntervalString:_student.arrive_time];
//        NSLog(@"timeInt ====== %d",self.timeInt);
        if (self.timeInt >= 300) {
            self.passLate_label.text = @"乘客迟到";
    ...
    [[NSUserDefaults standardUserDefaults] setObject:@"3" forKey:@"passLateStr"];
    [self.statu_button setTitle:@"开始行程" forState:UIControlStateNormal];

    }else if ([_student.order_state isEqualToString:@"4"]){
    [self.statu_button setTitle:@"安全送达(拍照)" forState:UIControlStateNormal];
    ...
    self.time_Label.text = _student.school_time;
...
__block int timeout=300 - self.timeInt; //倒计时时间
...
dispatch_source_set_event_handler(_timer, ^{
    ...
    if(timeout<=0){ //倒计时结束，关闭
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            weakSelf.passLate_label.text = @"乘客迟到";
            [weakSelf.passLate_label addGestureRecognizer:self.passLabelTap];
        });
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            //设置界面的按钮显示 根据自己需求设置
            weakSelf.passLate_label.text = [NSString stringWithFormat:@"%02d:%02d",minutes,seconds];
        });
...
- (void)didClickStatusButton{
    ...
    if ([self.statu_button.titleLabel.text isEqualToString:@"安全送达(拍照)"] ||[self.statu_button.titleLabel.text isEqualToString:@"安全送达"]) {
        //调取摄像机
...
- (void)passLabelTapDidClick{
    ...
    if ([self.passLate_label.text isEqualToString:@"乘客迟到"]) {
        
        self.singleAlertView =[[CYAlertView alloc]initWithFrame:CGRectMake(0, 0, DeviceWidth, DeviceHeight)];
        [self.singleAlertView setDoubleButton];
        self.singleAlertView.titleStr=@"乘客迟到";
        self.singleAlertView.alertStr=[NSString stringWithFormat:@"您是否取消订单"];
        self.singleAlertView.cancleButtonStr=@"否";
        self.singleAlertView.sureButtonStr=@"是";
        ...
        [window addSubview:self.singleAlertView];
        self.singleAlertView.phoneBlock = ^{//确认取消
            [weakSingleAlertView hideAlertView];
            if (weakSelf.delagte&&[weakSelf.delagte respondsToSelector:@selector(cancelBtnClick:)]) {
                [weakSelf.delagte cancelBtnClick:weakSelf.student.order_id];
            }
        };
        self.singleAlertView.cancleBlock = ^{//不取消
             [weakSingleAlertView hideAlertView];
