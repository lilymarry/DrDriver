...
@interface AfterSchoolTableViewCell()
...
@property (nonatomic, strong, readwrite) UIButton *statu_button;//状态
...
- (void)setStudent:(SLStudent *)student{
    _student = student;
    if (([_student.order_state intValue] > 0 && [_student.order_state intValue] < 5) || [_student.order_state isEqualToString:@"9"]) {
...
//order_state订单状态，0：司机未发车，1司机未到达，2未上车，3已上车，4行程中，5司机已送达，6家长确认送达，7司机取消，8乘客取消，9陪伴中
    if ([_student.order_state isEqualToString:@"0"]) {
     ...
     [self.statu_button setTitle:@"安全送达(拍照)" forState:UIControlStateNormal];
        if ([_student.accompany_type intValue] > 0) {//隐藏黄色按钮
          [self.passLate_label removeFromSuperview];
   ...
   }else if ([_student.order_state isEqualToString:@"5"] ||[_student.order_state isEqualToString:@"6"] ||[_student.order_state isEqualToString:@"7"]||[_student.order_state isEqualToString:@"8"]){
       self.statu_button.hidden = YES;
   }else if ([_student.order_state isEqualToString:@"9"]){
       [self.statu_button setTitle:@"安全送达(拍照)" forState:UIControlStateNormal];
   }
...
- (void)passLabelTapDidClick{
    ...
    if (self.delagte&&[self.delagte respondsToSelector:@selector(skipBtnClick:withType:)]) {
            [self.delagte skipBtnClick:self.student.order_id withType:@""];
        }
...
- (void)didClickTelButton{
    ...
    if (self.delagte && [self.delagte respondsToSelector:@selector(clickTellPhoneButton:)]) {
        [self.delagte clickTellPhoneButton:self.parent_phone];
    }
...
- (void)didClickStatusButton{
    ...
    if ([self.statu_button.titleLabel.text isEqualToString:@"安全送达(拍照)"]) {
        //调取摄像机
        if (self.delagte && [self.delagte respondsToSelector:@selector(clickPhotoButton:)]) {
            [self.delagte clickPhotoButton:self.student.order_id];
        }
        return;
    }
    if (self.delagte && [self.delagte respondsToSelector:@selector(clickStateButton:)]) {
        [self.delagte clickStateButton:self.student];
    }
